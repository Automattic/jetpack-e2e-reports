...
export default class Test extends BaseComponent {
	...
	async componentDidMount() {
		...
		this.setState( { weeks: this.filterData( this.state.rawData.stats.weekly ) } );
		this.setState( { months: this.filterData( this.state.rawData.stats.monthly ) } );
	...
	filterData( rawData ) {
		...
		entries.forEach( day => {
			day.failedRate = day.total > 0 ? ( ( day.failed / day.total ) * 100 ).toFixed( 2 ) : 0;
		} );
	...
	filterSummaryData() {
		...
		if ( this.state.filters.isTrunkOnly ) {
			Object.keys( this.state.rawData.stats.summary ).forEach( key => {
				summaryData[ key ] = this.state.rawData.stats.summary[ key ].trunk;
			} );
		} else {
			Object.keys( this.state.rawData.stats.summary ).forEach( key => {
				summaryData[ key ] = this.state.rawData.stats.summary[ key ].total;
			} );
		}
			...
			<div>
				...
				<div className="row justify-content-end">
					<div className="col-sm text-end">
						<small>updated { moment( this.state.rawData.lastUpdate ).fromNow() }</small>
					</div>
				</div>
				<hr />
