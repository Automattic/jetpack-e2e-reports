...
writeJson(
	sourceJson,
	`docs/data/tests-bk-${ moment( new Date() ).format( 'YYYY-MM-DD-hh-mm' ) }.json`
);
...
for ( const test of sourceJson.tests ) {
	...
	for ( const srcResult of test.results ) {
		...
		const existingTest = monthlyJson.tests.filter( t => t.name === test.name );
		...
		if ( existingTest.length === 0 ) {
			...
			const existingResult = existingTest[ 0 ].results.filter( r => r.time === srcResult.time );
			...
			if ( existingResult.length === 0 ) {
				...
				console.log(
					`Result already exists: test: ${ test.name }, result ${ JSON.stringify( srcResult ) }`
				);
			}
		}
