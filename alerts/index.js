...
( async function main() {
	...
	const rules = {
		...
		consecutive_failures_jetpack_production: async () =>
			await consecutiveFailures( 'jetpack-production', 5 ),
		consecutive_failures_jetpack_boost_production: async () =>
			await consecutiveFailures( 'jetpack-boost-production', 5 ),
		consecutive_failures_jetpack_social_plugin: async () =>
			await consecutiveFailures( 'jetpack-social-plugin', 5 ),
		consecutive_failures_jetpack_search_plugin: async () =>
			await consecutiveFailures( 'jetpack-search-plugin', 5 ),
